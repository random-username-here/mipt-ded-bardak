...
public:
	...
	void spawnPacman(BmClient *client)
	{
		...
		pacmen_.try_emplace(client, map_, client, animator_, assets_);
	}
  ...
  private:
	...
	void sendVisibleThreats(BmClient *client)
	{
		for (const auto &[_, ent] : map_->getEntityList()) {
			if (!isAliveEntityForPacman(ent) || dynamic_cast<Ghost *>(ent) == nullptr) {
				continue;
			}
			const Vec2i p = ent->getPosition();
			client->send(bmsg::SV_pacman_sees{p.x, p.y, static_cast<uint32_t>(ent->getID())});
		}
	}
