...
class KnightModule final : public BmServerModule {
    Timer   *tm    = nullptr;
    Level   *map   = nullptr;
    RoleMgr *roles = nullptr;
    anim::AnimationManager *animator = nullptr;
    AssetManager           *assets   = nullptr;
    KnightManager manager;

public:
    std::string_view id()    const override { return "ashww.bardak.uctl.knight"; }
    std::string_view brief() const override { return "Unit controller for knight"; }
    ...
    {
        manager.resolve();
        if (!roles->registerRole("knight", "knight", "knight", [this](BmClient *client) {
                select(client);
            })) {
            throw ModManager::Error("failed to register knight role");
        }
    }

    void onSetup(BmServer *server) override
    {
        server->registerPrefix("knight", this);
    }
    ...
    {
        if (!message.isCorrect() || !roles->clientHasRole(client, "knight")) {
            return;
        }

        if (message.header()->type == "move") {
            const auto move = bmsg::CL_knight_move::decode(message);
            if (move) {
...
private:
    void select(BmClient *client)
    {
        ...
        manager.spawnKnight(client);
    }
};

extern "C" Mod *modlib_create(ModManager *)
{
    return new KnightModule();
}
