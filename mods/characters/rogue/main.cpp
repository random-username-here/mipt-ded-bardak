...
#include "rogue_manager.hpp"

#include <string_view>

using namespace modlib;

class RogueModule final : public BmServerModule {
    Timer   *tm    = nullptr;
    Level   *map   = nullptr;
    RoleMgr *roles = nullptr;
    anim::AnimationManager *animator = nullptr;
    AssetManager           *assets   = nullptr;
    RogueManager            manager;

public:
    std::string_view id()    const override { return "ashww.bardak.uctl.rogue"; }
    std::string_view brief() const override { return "Unit controller for rogue skeleton"; }
    ...
    void onDepsResolved(ModManager *) override
    {
        ...
        if (!roles->registerRole("rogue", "rogue", "rogue", [this](BmClient *client) {
                select(client);
            })) {
            throw ModManager::Error("failed to register rogue role");
        }
    }
    ...
    {
        if (!message.isCorrect() || !roles->clientHasRole(client, "rogue")) {
            return;
        }

        if (message.header()->type == "move") {
            const auto move = bmsg::CL_rogue_move::decode(message);
            if (move) {
        ...
        if (message.header()->type == "use") {
            const auto use = bmsg::CL_rogue_use::decode(message);
            if (use) {
...
private:
    void select(BmClient *client)
    {
        ...
        manager.spawnRogue(client);
    }
};

extern "C" Mod *modlib_create(ModManager *)
{
    return new RogueModule();
}
