...
class ArcherClient : public ClientBase {
    bool    m_alive = true;
    int32_t m_hp = 0;
...
public:
    ArcherClient(const std::string &ini)
        : ClientBase(ini)
    {
        registerOnPrefix("archer", [this](const PanFrame &frame) {
            return handleArcherFrame(frame);
        });
    }
...
private:
    ...
    bool handleArcherFrame(const PanFrame &frame)
    {
        ...
        if (type == "tick") {
            if (!bmsg::SV_archer_tick::decode(msg)) {
                return false;
            }
        ...
        if (type == "hp") {
            const auto hp = bmsg::SV_archer_hp::decode(msg);
            if (!hp) {
                return false;
            }
        ...
        if (type == "at") {
            const auto at = bmsg::SV_archer_at::decode(msg);
            if (!at) {
                return false;
            }
        ...
        if (type == "root") {
            const auto root = bmsg::SV_archer_root::decode(msg);
            if (!root) {
                return false;
            }
        ...
        if (type == "enemy") {
            const auto enemy = bmsg::SV_archer_enemy::decode(msg);
            if (!enemy) {
                return false;
            }
        ...
        if (type == "wall") {
            const auto wall = bmsg::SV_archer_wall::decode(msg);
            if (!wall) {
                return false;
            }
        ...
        if (type == "item") {
            const auto item = bmsg::SV_archer_item::decode(msg);
            if (!item) {
                return false;
            }
        ...
        if (type == "ability") {
            const auto ability = bmsg::SV_archer_ability::decode(msg);
            if (!ability) {
                return false;
            }
    ...
    {
        if (!sendMessage(bmsg::CL_archer_use{ability, target})) {
            std::cerr << "send archer use failed\n";
            m_alive = false;
            return false;
        }
    ...
    bool sendMove(AiPos step)
    {
        ...
        if (!sendMessage(bmsg::CL_archer_move{
                static_cast<int8_t>(step.x),
                static_cast<int8_t>(step.y)
            })) {
            std::cerr << "send archer move failed\n";
            m_alive = false;
            return false;
        }
...
int main(int argc, char **argv)
{
    ...
    try {
        return ArcherClient(argv[1]).run() ? 0 : 1;
    }
