...
class RogueClient : public ClientBase {
    bool    m_alive = true;
    int32_t m_hp = 0;
...
public:
    RogueClient(const std::string &ini)
        : ClientBase(ini)
    {
        registerOnPrefix("rogue", [this](const PanFrame &frame) {
            return handleRogueFrame(frame);
        });
    }
...
private:
    ...
    bool handleRogueFrame(const PanFrame &frame)
    {
        ...
        if (type == "tick") {
            if (!bmsg::SV_rogue_tick::decode(msg)) {
                return false;
            }
        ...
        if (type == "hp") {
            const auto hp = bmsg::SV_rogue_hp::decode(msg);
            if (!hp) {
                return false;
            }
        ...
        if (type == "at") {
            const auto at = bmsg::SV_rogue_at::decode(msg);
            if (!at) {
                return false;
            }
        ...
        if (type == "root") {
            const auto root = bmsg::SV_rogue_root::decode(msg);
            if (!root) {
                return false;
            }
        ...
        if (type == "enemy") {
            const auto enemy = bmsg::SV_rogue_enemy::decode(msg);
            if (!enemy) {
                return false;
            }
        ...
        if (type == "wall") {
            const auto wall = bmsg::SV_rogue_wall::decode(msg);
            if (!wall) {
                return false;
            }
        ...
        if (type == "item") {
            const auto item = bmsg::SV_rogue_item::decode(msg);
            if (!item) {
                return false;
            }
        ...
        if (type == "ability") {
            const auto ability = bmsg::SV_rogue_ability::decode(msg);
            if (!ability) {
                return false;
            }
    ...
    {
        if (!sendMessage(bmsg::CL_rogue_use{ability, target})) {
            std::cerr << "send rogue use failed\n";
            m_alive = false;
            return false;
        }
    ...
    bool sendMove(AiPos step)
    {
        ...
        if (!sendMessage(bmsg::CL_rogue_move{
                static_cast<int8_t>(step.x),
                static_cast<int8_t>(step.y)
            })) {
            std::cerr << "send rogue move failed\n";
            m_alive = false;
            return false;
        }
...
int main(int argc, char **argv)
{
    ...
    try {
        return RogueClient(argv[1]).run() ? 0 : 1;
    }
