...
{
	static const char *colors[] = {ESC_RED, ESC_GRN, ESC_YLW, ESC_BLU, ESC_CYN, ESC_MGN};
...
bool ServerImpl::registerPrefix(std::string_view pref, modlib::BmServerModule *mod)
{
	...
	if (m_prefMapping.count(u))
		return false;
	m_prefMapping[u] = mod;
	logg() << "Plugin " << ESC_BLU << mod->id() << ESC_RST << " now responsible for prefix "
	       << ESC_GRN << pref << ESC_RST << '\n';
	return true;
}
...
void ServerImpl::listenPrefix(std::string_view pref, modlib::BmServerModule *mod)
{
	...
	logg() << "Plugin " << ESC_BLU << mod->id() << ESC_RST << " now listening messages for "
	       << ESC_GRN << pref << ESC_RST << '\n';
}
...
void ServerImpl::listenAll(modlib::BmServerModule *mod)
{
	...
	logg() << "Plugin " << ESC_BLU << mod->id() << ESC_RST
	       << " now listening all incoming messages\n";
}
...
void ServerImpl::mainloop()
{
	...
	while (1) {
		...
		if (!ev.data.ptr) {
			...
			m_clients[id]->logg() << ESC_MGN << "Client connected from " << buf << ":"
			                      << ntohs(addr.sin_port) << "\n"
			                      << ESC_RST;
